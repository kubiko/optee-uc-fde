...
static void print_help(void) {
    ...
    printf("\t\t\tPath to the created encrypted volume is passed as last argument.\n");
    printf("\n\t\t--post-unlock-cmd=<cmd>\n");
...
int handle_post_activation_step(const char *label, const char *post_unlock_cmd) {
    int ret = EXIT_SUCCESS;
    char post_unlock[256];
    snprintf(post_unlock, 256, "%s %s/%s", post_unlock_cmd, crypt_get_dir(), label);
    ret = system(post_unlock);
    if (ret) {
        if (WIFSIGNALED(ret))
...
static int format_and_add_key(struct crypt_device *cd,
    ...
    if (post_format_cmd) {
        snprintf(post_format, 256, "%s %s/%s", post_format_cmd, crypt_get_dir(), label);
        ret = system(post_format);
        if (ret) {
            if (WIFSIGNALED(ret))
